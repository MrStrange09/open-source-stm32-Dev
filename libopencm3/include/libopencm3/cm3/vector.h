 ...
 * pointers in the efm32_vector_table_t struct. */
 ...
 // #define is handled by the preprocessor before C compilation.
 // typedef is handled by the C compiler. typedef creates a real type alias.

 /*
 Constants/macros/register addresses -> #define
	Type names/struct names/function pointer types -> typedef
 */

typedef void (*vector_table_entry_t)(void);
//structure of function pointers that returns nothing and take no argument
typedef struct {
	...
	vector_table_entry_t irq[NVIC_IRQ_COUNT]; //can see more interrupts here
} vector_table_t;
